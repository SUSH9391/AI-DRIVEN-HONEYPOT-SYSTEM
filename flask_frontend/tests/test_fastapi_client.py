...
@pytest.mark.asyncio
async def test_score_attack_adds_service_token_and_jwt_header(mock_httpx_fastapi_client):
    mock_httpx_fastapi_client.return_value = {"attack_detected": True}
    
    await fastapi_client.score_attack(
        "sandbox1", "token1", {"k": "v"}, "/test", "127.0.0.1", jwt="test-jwt"
    )
